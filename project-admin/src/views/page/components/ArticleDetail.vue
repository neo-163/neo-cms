  ...
  <div class="createPost-container">
    <el-form ref="postForm" :model="postForm" :rules="rules" class="form-container">
      ...
      <sticky :z-index="10" :class-name="'sub-navbar '+postForm.status">
        ...
        <SourceUrlDropdown v-model="postForm.url_tag" style="margin-left: 10px;" />

        <el-button plain style="margin-left: 10px;" @click="displayJson">
          {{ showJsonText }} Json 数据
        </el-button>
      ...
      <div class="createPost-main-container">
        ...
        <el-form-item prop="content" style="margin-bottom: 30px;" v-if="showJson">
          <label class="json-textarea-text">Json 数据</label>
          <textarea class="form-control" id="json-textarea" rows="8" v-model="postForm.json"></textarea>
        </el-form-item>
...
import { fetchItem, createItem, editItem } from '@/api/page'
...
export default {
  ...
  methods: {
    ...
    createItemOption() {
      createItem(this.postForm).then(response => {
        ...
        if(response.code == 200) {
          this.$router.push('/page/edit/'+response.data.id);
        }
      }).catch(err => {
        console.log(err)
      })
    ...
    editItemOption() {
      editItem(this.postForm).then(response => {
        // console.log(response)
        this.returnArticleTigs(response)
    ...
    displayJson() {
      if(!this.showJson) {
        this.showJson = true
        this.showJsonText = '隐藏'
      } else {
        this.showJson = false
        this.showJsonText = '展示'
      }
    },
    submitForm() {
      ...
      if( (json.charAt(0) != '{' || json.substr(-1) != '}') && (json.charAt(0) != '[' || json.substr(-1) != ']') ) {
        this.notJson()
        return false
      }
